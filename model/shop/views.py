...
def add_visit(request):
    ...
    # 创建或更新拜访记录
    visit, created = models.Visit.objects.get_or_create(user=user, shop=shop, defaults={'notes': ''})
    if not created:
...
def visit_shop_list(request):
    ...
    for visit in visits_page:
        ...
        visit_data = {
            ...
            'shop_note': visit.shop.shop_note,  # 添加商店备注字段
...
def update_shop(request):
    ...
    try:
        ...
        with transaction.atomic():
            # 使用select_for_update锁定商店记录，防止并发修改
            shop = models.Shop.objects.select_for_update().get(id=shop_id)
            ...
            if 'tags' in request.data:
                ...
                if isinstance(tags_data, str):
                    ...
                    for tag_name in tag_names:
                        # 使用get_or_create的事务安全版本
                        tag, _ = models.Tag.objects.select_for_update().get_or_create(name=tag_name)
            ...
            # 更新其他字段
            for field in ['name', 'country', 'city', 'lat', 'lon', 'address', 'phone', 'email', 'level', 'shop_note']:
...
def shop_create(request):
    ...
    visited_value = request.data.get('visited')
    # 确保正确处理visited字段，将字符串"0"转换为False
    visited = False if visited_value == "0" or visited_value == 0 or visited_value == False or visited_value == "false" else bool(visited_value)
    tags = request.data.get('tags')
    shop_note = request.data.get('shop_note', '')

    # 使用事务管理，确保所有数据库操作作为一个原子单元执行
    from django.db import transaction
    
    try:
        # 开始事务
        with transaction.atomic():
            # 创建商店记录，使用select_for_update锁定行，防止并发修改
            ...
            if tags:
                for tag_id in tags:
                    try:
                        # 使用select_for_update锁定标签记录
                        tag = models.Tag.objects.select_for_update().get(id=tag_id)
                        models.ShopTag.objects.create(shop_id=shop, tag_id=tag)
                    except models.Tag.DoesNotExist:
                        continue
...
def delete_shop(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # 事务成功完成后，删除图片文件（文件系统操作不能回滚）
            if image_path and os.path.exists(image_path):
                try:
                    os.remove(image_path)
                except Exception as e:
                    # 记录错误但不中断流程，因为数据库操作已成功
